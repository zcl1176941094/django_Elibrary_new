    ...
    <div class="star">
        ...
        <div>
          <el-input
            ...
            :rows="4"/>
      <!-- 发布按钮 -->
          <span style="margin-right:1px;float: right;"><button @click="submit">发表评论</button></span>
        </div>
...
<br/><br/>
        ...
        <div class="comment_show">
            <p v-if="commentsList==[]">暂无评论</p>
            <ul class="list_group" v-if="commentsList!=[]">
                <li class="list-group-item" v-for="item in commentsList"  :key="item.stime">
                        <h3 class="first-userid">{{item.userid}}</h3>
                        <span class="first-time">{{item.stime | timeFormat() }}</span>
                        <p class="first-comment">{{item.evaluation }}</p>
                        <span class="first-right">评分：{{ item.grade }}</span>
                </li>
            </ul>
        </div>
